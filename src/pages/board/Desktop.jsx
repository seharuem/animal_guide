	...
	return (
		<div className='w-full border-collapse hidden sm:flex flex-col'>
			{table.getHeaderGroups().map((headerGroup) => (
				<div key={headerGroup.id} className='border-y-2 border-(--p) flex py-2 text-xs font-semibold'>
			...
			{table.getRowModel().rows.map((row) => (
				<div key={row.id} className='py-2 flex text-xs font-semibold border-b border-(--p)'>
			...
			{Array(Math.max(0, 20 - table.getRowModel().rows.length))
				.fill(0)
				.map((_, index) => (
					<div key={index} className='py-2 text-xs border-b border-(--p)'>
						아직 작성된 글이 없습니다.
					</div>
				))}
		</div>
