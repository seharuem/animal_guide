	...
	return (
		<div className='w-full border-collapse hidden sm:flex flex-col'>
			{table.getHeaderGroups().map((headerGroup) => (
				<div key={headerGroup.id} className='border-y-2 border-(--c) flex py-2 text-xs font-semibold'>
			...
			{table.getRowModel().rows.map((row) => (
				<div key={row.id} className='py-2 flex text-xs font-semibold border-b border-(--c)'>
